<?
function _search($term=null, $offset=null, $pages=null, $page_id=null, $search_name='', $search_value='', $b64_encode=0){
    ...
    global $slocation, $PESQUISA_SKUS_DIRECTOS;
    global $CONFIG_ORDEM;
    global $CACHE_HEADER_FOOTER;
    global $SOLR_ORDEM_FILTROS, $SOLR_VERSION;
    global $B2B, $IP_WHITELIST, $CACHE_KEY;
    global $COM_PROD_CONFIG_PESQUISA, $db_name_cms, $SOLR_WITH_RANK;
    ...
    if ($term!='' || $search_name!=''){
        ...
        $b64_encode = (int)$b64_encode;
    }else{
        $term   = params('term');
        $offset = (int)params('offset');
        $pages  = (int)params('pages');
        $page_id  = (int)params('page_id');
        $search_name = params('search_nm');
        $search_value = params('search_vl');              
        $b64_encode  = (int)params('b64_encode');
    }
    ...
    if($pages>1 && (!isset($_SESSION['SYS_CTRSR'][$id]) || $_SESSION['SYS_CTRSR'][$id]<$pages-1) && !is_numeric($userID) && !in_array($_SERVER['REMOTE_ADDR'], $ARR_IP_WHITELIST)){   
        ob_end_clean();
        header('HTTP/1.1 403 Forbidden');
        exit;
    }
    
    $_SESSION['SYS_CTRSR'][$id] = $pages;
    ...
    $scope['PRICE']   = $_SESSION['_MARKET']['lista_preco'];   
    $scope['FILTER']  = serialize($_SESSION['filter_active']['36']);
    $scope['ORDER']   = serialize($_SESSION['order_active']['36']);
    $scope['PROMO']               = implode(',', $PROMOS["promos"]); 
    $scope['SEG']                 = $_SESSION["segmentos"];    
     
    $CACHEID = $CACHE_KEY."PESQI_".implode('_', $scope) . implode("-",array($term,$offset,$pages,$page_id,$search_name,$search_value));
    
    
     if(is_array($_SESSION['EC_USER']['margem']) && count($_SESSION['EC_USER']['margem'])>0){
        $CACHEID .= md5(base64_encode(serialize($_SESSION['EC_USER']['margem'])));
    }
    
    if((int)$_SESSION['EC_USER']['descontos_exclusivos']>0){
        $CACHEID .= $_SESSION['EC_USER']['id']."|||".$_SESSION['EC_USER']['descontos_exclusivos'];
    }

    if((int)$_SESSION['EC_USER']['b2b_markup']>0){
        $CACHEID .= $_SESSION['EC_USER']['b2b_markup'];
    }
    ...
    if( (int)$SOLR == 1 && (int)$B2B == 1 && $search_name != '' && $search_value != '' ){    
        $file_search_filter = $_SERVER['DOCUMENT_ROOT'].'/api/controllers/_searchByFilterB2B.php';
        if(file_exists($file_search_filter)){
            include_once($file_search_filter);
            return call_user_func('_searchByFilterB2B', $offset, $pages, $page_id, $search_name, $search_value);
        }
    }
    ...
    if( $offset==0 && !is_numeric($userID) && !in_array($_SERVER['REMOTE_ADDR'], $ARR_IP_WHITELIST)){ 
        $key = 'SEARCH_SOLRF5_'.$_SERVER['REMOTE_ADDR'];            
        $x = 0 + @apc_fetch($key);
        if ($x>32) #2022-07-25 antes 8
        {
            $x++;
            apc_store($key, $x, 60*60);  #1hora  2022-07-25 antes 60*60*24
            ob_end_clean();
            header('HTTP/1.1 403 Forbidden');
            exit;
        } else {
            $x++;
            apc_store($key, $x, 30); #0.5min
        }
    }
    ...
    $sinonimos_pesquisa = array();
    if( (int)$B2B == 0) $sinonimos_pesquisa = getSinonimosPesquisa($term);
    ...
    if( (int)$B2B == 0 && (int)$SOLR > 0 ){
        ...
        if($response->responseHeader->status=="0"){
            foreach($response->spellcheck->suggestions[1]->suggestion as $key => $value){
                ...
                $res = explode(strtolower($term), $value);
        ...
        if(count($palavras)>0){
            ...
            # Shipping Express
            if( trim($_COOKIE['SYS_EXP_ZIP']) != '' && isset($_GET['shippingexpress']) ){
                ...
                foreach($response->response->docs as $key=>$value){
                    ...
                    if($prod["id"]>0){
                        ...
                        if( trim($_COOKIE['SYS_EXP_ZIP']) != '' && isset($_GET['shippingexpress']) ){
                            ...
                            if( trim($row_express['depositos']) != '' && (int)$_GET['shippingexpress'] == 1 ){
                                $prod['url'] = str_replace("id=36", "id=".$shipping_express_page, $prod['url']);
                                $temp[] = $prod;    
                            }elseif( trim($row_express['depositos']) == '' && (int)$_GET['shippingexpress'] == 0 ){
                                $temp[] = $prod;
                            }

                            if( count($temp) == $CONFIG_LISTAGEM_QTD ) break;
        ...
        $temp_products_info["COUNT"] = (int)$response->response->numFound;
        if( trim($_COOKIE['SYS_EXP_ZIP']) != '' && isset($_GET['shippingexpress']) ){
            $temp_products_info["COUNT"] = count($temp);
        }
        ...
        if(count($palavras)>1){
            ...
            /*$_query_regras = "AND (((registos.desc$LG LIKE '%".cms_escape(implode('%',$palavras))."%' OR registos.descritores$LG  REGEXP '" . implode('|', array_map(function($p) {return '[[:<:]]' . preg_quote($p, '/') . '[[:>:]]';}, $palavras)) . "' OR registos.nome$LG  LIKE '%".cms_escape(implode('%',$palavras))."%')  ";
            ...
            # 2023-09-19
            # Definido por Serafim que em B2B primeiro fazemos a pesquisa com as palarvras todas juntos e só se não obter resultados é que explodimos por espaços
            if((int)$B2B==0){
                ...
                # Aprovado pelo Serafim
                if((int)$PESQUISA_SKUS_DIRECTOS>0){
                    $_query_regras_arr_sk[] = " registos.sku LIKE '".cms_escape(implode('%',$palavras))."%' OR registos.sku_family LIKE '".cms_escape(implode('%',$palavras))."%' OR registos.sku_group LIKE '".cms_escape(implode('%',$palavras))."%' OR registos.ean LIKE '".cms_escape(implode('%',$palavras))."%' OR CONCAT(',', registos.ean, ',') LIKE '%,".cms_escape(implode('%',$palavras))."%,%' ";            
                    
                    /*$_query_order_arr[71]  ="when registos.sku like '".cms_escape(implode('%',$palavras))."%' then '70'";
                    $_query_order_arr[72]  ="when registos.sku_family like '".cms_escape(implode('%',$palavras))."%' then '70'";
                    $_query_order_arr[73]  ="when registos.sku_group like '".cms_escape(implode('%',$palavras))."%' then '70'";*/
                    ...
                    $_query_order_arr[74]  ="when registos.sku like '".cms_escape(implode('%',$palavras))."%' then '73'";
                    $_query_order_arr[75]  ="when registos.sku_family like '".cms_escape(implode('%',$palavras))."%' then '73'";
                    $_query_order_arr[76]  ="when registos.sku_group like '".cms_escape(implode('%',$palavras))."%' then '73'";

                    $_query_order_arr[77]  ="when registos.sku like '%".cms_escape(implode('%',$palavras))."%' then '75'";
                    $_query_order_arr[78]  ="when registos.sku_family like '%".cms_escape(implode('%',$palavras))."%' then '75'";
                    $_query_order_arr[79]  ="when registos.sku_group like '%".cms_escape(implode('%',$palavras))."%' then '75'";*/
            ...
            if((int)$PESQUISA_SKUS_DIRECTOS>0){
                ...
                $_query_order_arr[74]  ="when registos.sku like '".cms_escape($term)."%' then '73'";
                $_query_order_arr[75]  ="when registos.sku_family like '".cms_escape($term)."%' then '73'";
                $_query_order_arr[76]  ="when registos.sku_group like '".cms_escape($term)."%' then '73'";

                $_query_order_arr[77]  ="when registos.sku like '%".cms_escape($term)."%' then '75'";
                $_query_order_arr[78]  ="when registos.sku_family like '%".cms_escape($term)."%' then '75'";
                $_query_order_arr[79]  ="when registos.sku_group like '%".cms_escape($term)."%' then '75'";
        ...
        $CASE = "";
        if(count($_query_order_arr) > 0) $CASE = "CASE ".implode(" ",$_query_order_arr)." ELSE '9999' END ASC,";
        $CONFIG_ORDEM  = " score_relevancia DESC, $CASE ".$CONFIG_ORDEM_INIT;
                        
        
        $temp_products_info = array();
        if( count($palavras)>0 ) {     
        
            $_query_regras_inicial = $_query_regras;
            ...
            $temp_products_info = call_api_func('get_products', $page_id, $offset, $_query_regras, $pages,0,0,0,0,0,0,$add_score_sql);
            
            
            
            
            if($B2B==1 && count($palavras)>1 && $temp_products_info['COUNT']==0){
                ...
                ksort($_query_order_arr);
        				...
        				$CONFIG_ORDEM = "CASE ".implode(" ",$_query_order_arr)." ELSE '9999' END ASC,".$CONFIG_ORDEM_INIT;
                
                
                
                $temp_products_info = array();

                if( count($_query_regras_arr)>0 ) $_query_regras_inicial .= " OR (( ".implode(") AND (",$_query_regras_arr)." ) ) ";  
                if( count($_query_regras_arr_sk)>0 ) $_query_regras_inicial .= " OR (( ".implode(") OR (",$_query_regras_arr_sk)." ) ) )";              
    
                if(count($sinonimos_pesquisa) > 0){
                    $_query_regras_inicial .= $_query_regras_sin;
                    $_query_regras_inicial .= " OR (( ".implode(") AND (",$_query_regras_arr_sin)." ) ) )";   
                }   
                
                $_query_regras_inicial .= ")";     
                    

                $temp_products_info = call_api_func('get_products', $page_id, $offset, $_query_regras_inicial, $pages);
    ...
    if((int)$B2B>0){
        ...
        foreach($temp_products_info['PRODUCTS'] as $k => $v){
            ...
            if( count($matriz['colunas']) == 1 && count($matriz['linhas']) == 1 && (int)$v['uncataloged_stock'] == 0 ){
                ...
                $temp_products_info['PRODUCTS'][$k]['discount_levels']        = product_has_discount_levels($userOriginalID, $temp_products_info['PRODUCTS'][$k]['sku']);
            }
    ...
    $resp['autocompleter']    = $autocompleter;
    ...
    $save_resp = serialize($resp);
    
    
    if(count($resp['results']['items'])>0) $fx->_SetCache($CACHEID, $save_resp, $CACHE_HEADER_FOOTER);
