...
function _getUserVehicleHistory(){
    ...
    $vehicles_res = cms_query("SELECT `pecas_veiculos_historico`.`id` as `id`, 
                                ...
                                FROM `pecas_veiculos_historico`
                                  LEFT JOIN `pecas_veiculos_utilizadores` ON `pecas_veiculos_utilizadores`.`veiculo_id`=`pecas_veiculos_historico`.`veiculo_id` AND `pecas_veiculos_utilizadores`.`user_id`=".$user_id."
                                  JOIN `pecas_veiculos` ON `pecas_veiculos`.`id`=`pecas_veiculos_historico`.`veiculo_id`
                                ...
                                WHERE `pecas_veiculos_historico`.`user_id`=".$user_id." 
                                    AND `pecas_veiculos_utilizadores`.`veiculo_id` IS NULL 
                                    AND `pecas_veiculos_historico`.`data` > NOW() - INTERVAL 30 DAY
                                ORDER BY `pecas_veiculos_historico`.`data`
                                LIMIT 6");
